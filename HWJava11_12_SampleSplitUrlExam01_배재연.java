...
class SampleSplitUrlExam01
{
	public static void main(String[] args) 
	{
		String url = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=0&ie=utf8&query=html";	// URL
		String protocol;
		String domain;
		String path;
		String query;
		...
		String[][] queryNameValue = new String[splitedQuery.length][2];

		for (int i = 0; i < queryNameValue.length; i++)
		{
			String[] temp = splitedQuery[i].split("=");
			queryNameValue[i][0] = temp[0];
			queryNameValue[i][1] = temp[1];
		}
		...
		System.out.println("QueryString 개수 : " + splitedQuery.length);
		for (int i = 0; i < queryNameValue.length; i++)
		{
			System.out.print("QueryString 변수명 : " + queryNameValue[i][0] + "\t\t/ ");
			System.out.println("변수값 : " + queryNameValue[i][1]);
		}
