/*
HWJava21_11_LambdaEx02_배재연.java
*/
...
public class FunctionToLambdaExpression {
	public static void main(String[] args) {
		...
		B b3 = (a) -> {System.out.println("입력 O 리턴 X 함수");};	// 입력매개변수타입 생략 가능
		B b4 = (a) -> System.out.println("입력 O 리턴 X 함수");	// 1줄 명령이면 중괄호 생략 가능
		B b5 = a -> System.out.println("입력 O 리턴 X 함수");	// 입력매개변수가 1개일 때 소괄호도 생략 가능(타입도 생략 필)
		// 3. 입력 X, 리턴 O
		// 3-1. 익명 이너 클래스 방식
		C c1 = new C() {
			@Override
		...
		C c3 = () -> 4;	// return이 생략되면 중괄호도 함께 생략해야 함.
		// 4. 입력 O, 리턴 O
		// 4-1. 익명 이너 클래스 방식
		D d1 = new D() {
			@Override
